...
const MAX_BASE64_CHARS = 24_000_000; // ~18MB binary
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const publicPath = `public/files/cms/${folder}/${yyyy}/${mm}/${fileNameBase}-${timestamp}.pdf`;

  try {
    const config = loadGithubConfig();
    const message = `cms: upload file ${folder}/${fileNameBase}`;
